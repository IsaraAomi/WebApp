...
def delete_item(item_id: int) -> dict[str, str]:
    if item_id not in grocery_list:
        raise HTTPException(status_code=404, detail="Item not found.")
    del_item: ItemPayload = grocery_list[item_id]
    del grocery_list[item_id]
    return {"result": f"Item({del_item.item_name}) deleted."}
...
def remove_quantity(item_id: int, quantity: int) -> dict[str, str]:
    ...
    if grocery_list[item_id].quantity <= quantity:
        ...
        pre_quantity: int = grocery_list[item_id].quantity
        post_quantity: int = pre_quantity - quantity
        grocery_list[item_id].quantity = post_quantity
    return {
        "result": f"Item({del_item.item_name}) {quantity} removed: {pre_quantity} -> {post_quantity}"
    }
